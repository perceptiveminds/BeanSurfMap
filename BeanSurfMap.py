...
conditionTypes = ["", "flat", "very poor", "poor", "poor to fair", "fair", "fair to good", "good", "very good",
                  "good to epic", "epic"]
...
for eachDay in tideReport["tide"]["tideSummary"]:
    if int(eachDay["date"]["mday"]) == datetime.datetime.now().day and int(
            eachDay["date"]["hour"]) > earliestSurfHour and int(
            eachDay["date"]["hour"]) < latestSurfHour:  # only add tide times during the day
...
class SurfSpot:
    ...
    def printReport(self):
        ...
        for day in range(0, daysInReport):
            reportText = reportText + str(self.heightsMin[day]) + "-" + str(self.heightsMax[day]) + " ft. " + str(
                conditionTypes[self.regionalConditions[day]]) + "  "
        print reportText
...
spots = [lindamar, bolinas, mavericks, oceanBeach, fourMile, steamerLane,
         pleasurePoint]  # same order as the surf artwork
